...
class JWTAuthenticationMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if (request.method == 'OPTIONS') | (request.path.startswith('/auth') and request.path.endswith('/changepassword') == False) | request.path.startswith('/space_id/create') | request.path.startswith('/space_id/banner'):
            return
